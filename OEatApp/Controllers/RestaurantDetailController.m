...
@interface RestaurantDetailController ()<UITableViewDataSource,UITableViewDelegate>
{
    RestProfileObj* restProfileObj;
...
- (void)initDataObj
{
    ...
    restProfileObj = [[RestProfileObj alloc] init];
    [restProfileObj initObj];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self reqRestaurantProfile:_restId];
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self reqRestaurantReview:_restId];
    });
}
...
- (void)likeBtnAct
{
    bLikeState = !bLikeState;
    if (bLikeState) {
        [likeBtn setImage:[UIImage imageNamed:@"fav"] forState:UIControlStateNormal];
    }else{
        [likeBtn setImage:[UIImage imageNamed:@"favno"] forState:UIControlStateNormal];
    }
    [self reqRestLike:bLikeState];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    switch (section) {
        ...
        case 0:
            ...
            if (bProfile) {
                label.text = restProfileObj.Name;
            }else{
                label.text = @"Loading...";
            }
...
{
    static NSString *RestProfileInfoCell = @"RestProfileInfoCell";
    static NSString *RestProfileSupplyCell = @"RestProfileSupplyCell";
    static NSString *RestProfileTrainingCell = @"RestProfileTrainingCell";
    static NSString *RestProfileInspectCell = @"RestProfileInspectCell";
    static NSString *RestProfileReviewCell = @"RestProfileReviewCell";
    
    UITableViewCell *cell;
    UILabel *_lbl;
    NSString *_strTmp;
    RestReviewObj *_reviewObj;
    NSString *keyName;
    __block UIImageView *_imv;
    switch (indexPath.section) {
        case 0:
        {
            cell = [tableView dequeueReusableCellWithIdentifier:RestProfileInfoCell];
            if (!cell)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"RestProfileInfoCell" owner:nil options:nil] lastObject];
            }
            _lbl = (UILabel*)[cell viewWithTag:1];
            [_lbl setText:restProfileObj.OpenTime];
            
            _lbl = (UILabel*)[cell viewWithTag:2];
            [_lbl setText:restProfileObj.Address];
            
            _lbl = (UILabel*)[cell viewWithTag:3];
            [_lbl setText:restProfileObj.Intro];
            
            _lbl = (UILabel*)[cell viewWithTag:4];
            [_lbl setTextColor:[UIColor colorWithHexString:@"#68bf60"]];
//            _strTmp = [NSString stringWithFormat:@"%@",restProfileObj.RestLike];
            _strTmp = [NSString stringWithFormat:@"%ld",(long)likeNum];
            [_lbl setText:_strTmp];
            
            likeBtn = (UIButton*)[cell viewWithTag:5];
            if (![likeBtn respondsToSelector:@selector(likeBtnAct)]) {
                [likeBtn addTarget:self action:@selector(likeBtnAct) forControlEvents:UIControlEventTouchUpInside];
        ...
        case 1:
        {
            cell = [tableView dequeueReusableCellWithIdentifier:RestProfileSupplyCell];
            if (!cell)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"RestProfileSupplyCell" owner:nil options:nil] lastObject];
            }
        ...
        case 2:
        {
            cell = [tableView dequeueReusableCellWithIdentifier:RestProfileTrainingCell];
            if (!cell)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"RestProfileTrainingCell" owner:nil options:nil] lastObject];
            }
        ...
        case 3:
        {
            cell = [tableView dequeueReusableCellWithIdentifier:RestProfileInspectCell];
            if (!cell)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"RestProfileInspectCell" owner:nil options:nil] lastObject];
            }
        ...
        case 4:
        {
            cell = [tableView dequeueReusableCellWithIdentifier:RestProfileReviewCell];
            if (!cell)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"RestProfileReviewCell" owner:nil options:nil] lastObject];
            }
...
- (void) configRestData
{
    ...
    if ([restProfileObj.SupplierStr length] > 10) {
        _jsonstr = [restProfileObj.SupplierStr stringByReplacingOccurrencesOfString:@"#" withString:_replaceStr];
    ...
    if ([restProfileObj.TrainingStr length] > 10) {
        _jsonstr = [restProfileObj.TrainingStr stringByReplacingOccurrencesOfString:@"#" withString:_replaceStr];
    ...
    if ([restProfileObj.SanitaryStr length] > 10) {
        _jsonstr = [restProfileObj.SanitaryStr stringByReplacingOccurrencesOfString:@"#" withString:_replaceStr];
...
- (void) reqRestaurantProfile:(NSString*)restId
{
    ...
    [body appendData:[@"interface_code=restaurantProfile&" dataUsingEncoding:NSUTF8StringEncoding]];
    
    NSString *_tmpStr = [NSString stringWithFormat:@"RestId=%@&",restId];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *indata, NSError *error) {
        ...
        NSLog(@"reqRestaurantProfile: %@",responseText);
        ...
        if (!error && responseCode == 200) {
            ...
            if (res && [res isKindOfClass:[NSDictionary class]]) {
                ...
                [self handleReqRestProfile:jsonDict];
            }
        }
    }];
...
- (void) handleReqRestProfile:(NSDictionary*)dictData
{
    ...
    if ([_ret isEqualToString:@"0"]) {
        ...
        restProfileObj.Logo = [dictData objectForKey:@"Logo"];
        restProfileObj.Name = [dictData objectForKey:@"Name"];
        restProfileObj.City = [dictData objectForKey:@"City"];
        restProfileObj.District = [dictData objectForKey:@"District"];
        restProfileObj.Address = [dictData objectForKey:@"Address"];
        restProfileObj.OpenTime = [dictData objectForKey:@"OpenTime"];
        restProfileObj.Intro = [dictData objectForKey:@"Intro"];
        restProfileObj.Type = [dictData objectForKey:@"Type"];
        
        restProfileObj.RestBadgeOne = [dictData objectForKey:@"RestBadgeOne"];
        restProfileObj.RestBadgeTwo = [dictData objectForKey:@"RestBadgeTwo"];
        restProfileObj.RestBadgeThree = [dictData objectForKey:@"RestBadgeThree"];
        restProfileObj.RestBadgeFour = [dictData objectForKey:@"RestBadgeFour"];
        restProfileObj.RestLike = [dictData objectForKey:@"RestLike"];
        restProfileObj.RestPrice = [dictData objectForKey:@"RestPrice"];
        
        //SupplierStr,TrainingStr,SanitaryStr
        restProfileObj.SupplierStr = [dictData objectForKey:@"SupplierStr"];
        restProfileObj.TrainingStr = [dictData objectForKey:@"TrainingStr"];
        restProfileObj.SanitaryStr = [dictData objectForKey:@"SanitaryStr"];
        
        likeNum = [restProfileObj.RestLike integerValue];
    ...
    if ([SupplierLogoArray count]>0 || [TrainingLogoArray count]>0 || [SanitaryLogoArray count]>0) {
...
- (void)updateCacheImages
{
    ...
    keyNum = [SupplierLogoArray count];
    for (int i = 0; i < keyNum; i++) {
        cacheKey = SupplierLogoArray[i];
    ...
    keyNum = [TrainingLogoArray count];
    for (int i = 0; i < keyNum; i++) {
        cacheKey = TrainingLogoArray[i];
    ...
    keyNum = [SanitaryLogoArray count];
    for (int i = 0; i < keyNum; i++) {
        cacheKey = SanitaryLogoArray[i];
...
- (void) reqRestaurantReview:(NSString*)restId
{
    NSMutableString *URLPath = [NSMutableString stringWithString:[[AccountManager theAccountManager] getRestUrl]];
    ...
    NSMutableData *body = [NSMutableData data];
    [body appendData:[@"interface_code=restaurantReview&" dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *indata, NSError *error) {
        NSInteger responseCode = [(NSHTTPURLResponse *)response statusCode];
        NSString *responseText = [[NSString alloc] initWithData:indata encoding:NSUTF8StringEncoding];
        NSLog(@"reqRestaurantReview: %@",responseText);
        
        if (!error && responseCode == 200) {
            id res = [NSJSONSerialization JSONObjectWithData:indata options:NSJSONReadingMutableContainers error:nil];
            if (res && [res isKindOfClass:[NSDictionary class]]) {
                NSDictionary *jsonDict = [[NSDictionary alloc] initWithDictionary:(NSDictionary *)res];
                [self handleReqRestReview:jsonDict];
            }
        }
    }];
...
- (void) handleReqRestReview:(NSDictionary*)dictData
{
    NSDictionary *_dataItem;
...
- (void) reqRestLike:(BOOL)bLike
{
    ...
    NSMutableData *body = [NSMutableData data];
    if (bLike) {
        [body appendData:[@"interface_code=plusRestLike&" dataUsingEncoding:NSUTF8StringEncoding]];
    }else{
        [body appendData:[@"interface_code=minusRestLike&" dataUsingEncoding:NSUTF8StringEncoding]];
    }
    
    NSString *_tmpStr = [NSString stringWithFormat:@"RestId=%@&",_restId];
    [body  appendData: [_tmpStr dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *indata, NSError *error) {
        NSInteger responseCode = [(NSHTTPURLResponse *)response statusCode];
        NSString *responseText = [[NSString alloc] initWithData:indata encoding:NSUTF8StringEncoding];
        NSLog(@"reqRestLike: %@",responseText);
...
- (void) handleReqLike:(NSDictionary*)dictData
{
    NSString *_ret = dictData[@"ret"];
